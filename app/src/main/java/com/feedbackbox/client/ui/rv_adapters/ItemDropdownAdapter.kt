...
import com.feedbackbox.client.databinding.ItemRvDropdownBinding
...
class ItemDropdownAdapter(
    ...
    inner class ViewHolder(itemView: View): RecyclerView.ViewHolder(itemView){
        fun bind(item: ItemDropdown, listener: ItemListener?, position: Int) = with(itemView){
            val binding = ItemRvDropdownBinding.bind(itemView)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return ViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.item_rv_dropdown, parent, false))
    }
